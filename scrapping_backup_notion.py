    ...
    try:
        ...
        WebDriverWait(driver, 30).until(
            EC.presence_of_element_located(
                (By.XPATH, '//*[@placeholder="Enter your email address..."]')
            )
        )
        # Email
        ...
        password_input = driver.find_element(
            By.XPATH, '//*[@placeholder="Enter your password..."]'
        )
        ...
        WebDriverWait(driver, 10).until(
            EC.presence_of_element_located(
                (By.XPATH, '//div[text()="Settings & members"]')
            )
        )
...
def generate_export():
    ...
    settandmembers_div = WebDriverWait(driver, 5).until(
        EC.presence_of_element_located((By.XPATH, '//div[text()="Settings & members"]'))
    )
    ...
    element = driver.find_element(
        By.XPATH, '//div[text()="Export all workspace content"]'
    )
    ...
    try:
        ...
        zip_file_locator = (
            By.XPATH,
            f"//a[contains(@href, '.zip') and not(contains(@href, '.crdownload'))]",
        )
...
def process_output(filename_path):
    new_original_backup_path = os.path.join(
        TARGET_PATH, os.path.basename(filename_path)
    )
            ...
            os.remove(
                ...
                + datetime.strftime(
                    min(
                        [
                            datetime.strptime(
                                str(back.split("export-")[1].replace(".zip", "")),
                                "%d-%m-%Y",
                            )
                            for back in backups
                        ]
            ...
            with zip_ref.open(internal_zip_filename) as internal_zip:
                with zipfile.ZipFile(internal_zip, "r") as internal_zip_ref:
                    ...
                    extracted_folder_name = os.path.splitext(
                        os.path.splitext(internal_zip_filename)[0]
                    )[0].replace("-Part-1", "")
    ...
    for root, dirs, files in temp_dirs:
        for i in dirs:
            ...
            try:
                ...
                with open(
                    os.path.join(TARGET_PATH, "log.txt"), "a", encoding="utf8"
                ) as f:
                    f.write(directory + "\n")
...
if __name__ == "__main__":
    ...
    logging.basicConfig(
        filename=os.path.join(data["DEBUG_PATH"], f"debug-{log_dat}.log"),
        level=logging.INFO,
    ...
    try:
        ...
        logging.exception(
            "An error occurred in the download phase of the webdriver: %s", e
        )
    ...
    try:
        ...
        logging.exception(
            "An error has occurred in the initialization phase of the webdriver: %s", e
        )
